...
class TypeTest {
  ...
  @Test
  def functionEquals {
    val f1 = FunctionType(IntType(32), Nil, List(IntType(32), FloatType(32)))
    val f2 = FunctionType(IntType(32), Nil, List(IntType(32), FloatType(32)))
    assertTrue(f1 == f2)
    assertEquals(f1.hashCode, f2.hashCode)
  }
  ...
  def nullSubtypePointer {
    val t1 = NullType
    val t2 = PointerType(UnitType, ReferenceType.NULLABLE)
    assertTrue(t1.isSubtypeOf(t2, module))
  }
